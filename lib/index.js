      ...
      makeHandler: function makeHandler(eventName, whitelist, opts) {
        ...
        return (() => {
          var _ref = _asyncToGenerator(function* (data, respond) {
            ...
            if (!data || !_.isString(data.action)) return respond(new BadRequestError());
            let action = data.action,
                params = data.params;

            if (whitelist && !svc.checkWhitelist(action, whitelist)) return respond(new ServiceNotFoundError(action));
            try {
